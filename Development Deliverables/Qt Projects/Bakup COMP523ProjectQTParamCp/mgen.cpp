...
//constructor
MGen::MGen(map<std::pair<QString,QString>,QString> & hmap, QString &filename):hmap(hmap),filename(filename){
}
...
void MGen::generateClass() {
    ...
    headerstream.open(MODELFILENAME,ios::out);
    ...
    sourcestream.open("model.cpp",ios::out);
...
void MGen::genSource(ofstream & sourcestream){
    sourcestream<<"#include \"mode.h\";"<<endl<<endl;
...
void MGen::genGet(map<std::pair<QString,QString>,QString>::const_iterator & it, ofstream & stream,bool headeronly) {
   ...
   stream<<"\t"<<it->second.toUtf8().constData()<<" "<<(headeronly?"":"Model::")
              <<"get"
